...
`ui_in`, `uo_out`, and `uio` byte-lane assignments plus the planned UART
console, SPI RAM, SPI target, QSPI, I2C, and LED/debug variants.
...
For later USB-visible debug, use a separate UART console variant on
`ui_in[3]`/`uo_out[4]`. The UART console is deferred from this smoke island
because the full UART RX/TX path exceeded the one-tile hardening area budget.
